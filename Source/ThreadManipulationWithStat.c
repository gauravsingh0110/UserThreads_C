...
int main_thread_id;
struct timeval go_tv,cleaup_tv;
...
void semSignal (semaphore * s)
{
	...
	{
		node->thread->tstate.state = READY;
		no_of_threads_ready ++;
...
void semWait(semaphore * s)
{
	...
	node->thread->tstate.state = WAITING;
	YieldCPU();
}
...
//	while(node != NULL)
//	{
//		if(node->thread->tstate.state == SLEEPING)
...
int CreateThread(void (*f)(void))
{
	...
	th_node->thread->tstate.state = READY;
//	th_node->thread->tstate.avr_exec_time=0;
	th_node->thread->tstate.avr_wait_time=0;
	th_node->thread->tstate.no_of_bursts=0;
	th_node->thread->tstate.total_exec_time=0;
	th_node->thread->tstate.total_sleep_time=0;
...
struct timeval dtvstart , dtvstop; // to calculate execution time
...
void Dispatch(int sig)
{
	...
	long s,e,t;
	AGThreadQNode *node;
	gettimeofday(&dtvstop , NULL);
	s=(dtvstart.tv_sec%10)*SECOND + dtvstart.tv_usec;
	e=(dtvstop.tv_sec%10)*SECOND + dtvstop.tv_usec;
	if(s>e)
	{
		t=e-s+10000000;
		runningThreadNode->thread->tstate.total_exec_time=runningThreadNode->thread->tstate.total_exec_time+t;
	...
	while(node != NULL)
	{
		node->thread->tstate.avr_wait_time=node->thread->tstate.avr_wait_time+t;
		node=node->next;
	}
	//printf("Traversing in Dispatch\n");

	//printf("old running thread id %d\n" , runningThreadNode->thread->tid);
	if(runningThreadNode->thread->tstate.state == RUNNING)
	{
	//	printf("inside special loop\n");
	runningThreadNode->thread->tstate.state = READY;
	no_of_threads_ready ++;
	...
	{
		printf("ALL Threads finished .. Ready List is Empty\n");//this should never be printed
		CleanUp();
	}
	runningThreadNode->thread->tstate.no_of_bursts=runningThreadNode->thread->tstate.no_of_bursts+1;
	no_of_threads_ready --;
	runningThreadNode->thread->tstate.state = RUNNING;
...
void Go()
{
	gettimeofday(&go_tv,NULL);
	main_thread_id=CreateThread(mainThread);
...
int DeleteThread(int id)
{
	...
	{
		runningThreadNode->thread->tstate.state = DEAD;
...
//	traverse(&readyHead);
	if(runningThreadNode->thread->tid == id)
	{
		runningThreadNode->thread->tstate.state = SUSPENDED;
	...
	{	//traverse(&readyHead);
		...
		{
			if(node->thread->tstate.state == SUSPENDED)
				return id;
			else
				return -1;
		}
...
//					return -1;
		...
		node->thread->tstate.state=SUSPENDED;
...
int GetStatus(int id, status * stat)
{
	...
	stat=malloc(sizeof(node->thread->tstate));
	*stat = node->thread->tstate;
	return id;
}
...
void uSleepThread(int usec)
{
	...
	else
	{
		...
		runningThreadNode->thread->tstate.state = SLEEPING;
		runningThreadNode->thread->tstate.total_sleep_time = runningThreadNode->thread->tstate.total_sleep_time + runningThreadNode->thread->sleep_time;
		if(no_of_sleeping_threads>0)
				reduceTime();
		else
		{
...
{
	AGThreadQNode *node , *temp;
	gettimeofday(&cleaup_tv,NULL);
	int left_threads;
	printf("\nTotal number of threads created : %d",total_no_of_threads-1);
	printf("\nTotal number of threads deleted : %d",no_of_threads_deleted);
	//if(runningThreadNode->thread->tid == main_thread_id)
	printf("\nTotal number of threads currently : %d",left_threads=total_no_of_threads-no_of_threads_deleted-1);
	//else
	//printf("\nTotal number of threads currently : %d",total_no_of_threads-no_of_threads_deleted-1);
	printf("\nElapased time between go and cleanup : %ld  ",(cleaup_tv.tv_sec-go_tv.tv_sec)*1000);
	if(left_threads>0)
	printf("\n\nThread Statistics : \n");
	...
	{
		printf("\nThread id : %d\n", runningThreadNode->thread->tid);
		printf("Number of bursts : %ld\n" , runningThreadNode->thread->tstate.no_of_bursts);
		printf("Total Execution time : %ld usec\n" ,runningThreadNode->thread->tstate.total_exec_time);
		printf("Total Sleep time : %ld usec\n" , runningThreadNode->thread->tstate.total_sleep_time);
		printf("Average Execution time : %ld usec\n" , runningThreadNode->thread->tstate.total_exec_time/runningThreadNode->thread->tstate.no_of_bursts);
		printf("Average Waiting time : %ld usec\n" , runningThreadNode->thread->tstate.avr_wait_time/runningThreadNode->thread->tstate.no_of_bursts);
	}
	free(runningThreadNode);
	runningThreadNode=NULL;
	...
	while(node != NULL)
	{
		temp = node;
		node = node -> next;
		if(temp->thread->tid == main_thread_id)
			continue;
		printf("\nThread id : %d\n", temp->thread->tid);
		printf("Number of bursts : %ld\n" , temp->thread->tstate.no_of_bursts);
		printf("Total Execution time : %ld usec\n" , temp->thread->tstate.total_exec_time);
		printf("Total Sleep time : %ld usec\n" , temp->thread->tstate.total_sleep_time);
		printf("Average Execution time : %ld usec\n" , temp->thread->tstate.total_exec_time/temp->thread->tstate.no_of_bursts);
		printf("Average Waiting time : %ld usec\n" , temp->thread->tstate.avr_wait_time/temp->thread->tstate.no_of_bursts);
		free(temp);
	}
	...
	printf("\nExitting ....\n");
	exit(0);
	//have to write more functions and CleanUp things before final submission
}
